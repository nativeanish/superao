...
const Metamask: React.FC<MetaMaskLogoProps> = ({ height = 20, width = 20 }) => {
  // Calculate the aspect ratio
  const aspectRatio = 318.6 / 318.6;

  // If only height is provided, calculate width
  const calculatedWidth =
    typeof height === "number" ? height * aspectRatio : width;

  // If only width is provided, calculate height
  const calculatedHeight =
    typeof width === "number" ? width / aspectRatio : height;
